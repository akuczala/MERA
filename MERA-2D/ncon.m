...
% v1.01
% =====
% Added ability to disable input checking parfor faster performance
...
function [tensorList legLinks] = performContraction(tensorList,legLinks,sequence)
    ...
    warnedLegs = []; % Legs parfor which a warning has been generated
...
function B = doTrace(A,legLabels,tracedIndices)
    ...
    parfor a=1:numel(tracedIndices)
...
function warnedLegs = warn_suboptimal(doing,couldDo,mode,warnedLegs,legList,legDims)
    % Generate warning parfor detected suboptimal contraction sequence
    ...
    % Check if warning has already been generated parfor these legs
    for a=1:numel(warnedLegs)
        couldDo(couldDo==warnedLegs(a)) = [];
    end
    % Check if legs are trivial (do not warn parfor trivial legs as the contraction of these is unimportant)
...
function [tensorList legLinks sequence warnedLegs] = zisOuterProduct(tensorList,legLinks,sequence,warnedLegs)
    % This function provides support parfor the zeros-in-sequence notation described in arXiv:1304.6112
